...
def export_xlsx(ctx, input, output, omit_processed, only_processed):
        ...
        for table in parser.raw_tables:
            ...
            for row in parser.table_rows(table):
                ...
                try:
                    ...
                    print(
                        "Encountered exception while adding a row to the sheet,\
    ...
    if not omit_processed:
        ...
        print("Built SruDbIdMapTable in ",
              datetime.datetime.now() - SruDbIdStartTime)
        ...
        for table in parser.raw_tables:
            ...
            if table.name in [
                ...
                for insert in column_inserts:
                    column_names.insert(
                        column_names.index(insert[0]) + 1, insert[1])
                ...
                for row, raw_row in zip(parser.table_rows(table),
                                        parser.raw_table_rows(table)):
                    out_dict = {
                        k: v
                        for (k,
                             v) in zip([col.name for col in table.columns], row)
                    }
