...
class TestParseEseValues:
    ...
    def test_double(self):
        assert ESEUtils.parse_ese_value(b'\x00\x00\x00\x00\x00\x00\xf0\xbf',
                                        7) == -1
        assert ESEUtils.parse_ese_value(b"\x00\x00\x00\x00\x08\x28\x0a\x41",
                                        7) == 0x034501

    def test_datetime(self):
        # FILETIME:
        target_date = datetime.datetime(2010, 6, 29, 9, 47, 42, 754212)
        parsed_date = ESEUtils.parse_ese_value(
            b'\x01\xcb\x17\x70\x1e\x9c\x88\x5a'[::-1], 8)
    ...
    def test_binary_largebinary(self):
        assert ESEUtils.parse_ese_value(b'\xde\xad\xbe\xef', 9) == repr(
            b'\xde\xad\xbe\xef')  # binary
        assert ESEUtils.parse_ese_value(b'\xde\xad\xbe\xef', 11) == repr(
            b'\xde\xad\xbe\xef')  # large binary

    def test_text_largetext(self):
        assert ESEUtils.parse_ese_value(b'ham sandwiches',
                                        10) == "ham sandwiches"  # text
        assert ESEUtils.parse_ese_value(b'sam handwiches',
                                        12) == "sam handwiches"  # large text
    ...
    def test_signed_long_long(self):
        assert ESEUtils.parse_ese_value(b"\xff\xff\xff\xff\xff\xff\xff\xff",
                                        15) == -1
        assert ESEUtils.parse_ese_value(b"\x01\x45\x03\x00\x00\x00\x00\x00",
                                        15) == 0x034501
